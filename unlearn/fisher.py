...
def fisher_information_martix(model, train_dl):
    model.eval()
    fisher_approximation = []
    for parameter in model.parameters():
        fisher_approximation.append(torch.zeros_like(parameter).cuda())
    total = 0
...
def hessian(dataset, model, loss_fn):
    model.eval()
    ...
    for inp, mask, label in tqdm(train_loader):
        inp = inp.cuda()
        mask = mask.cuda()
        label = label.cuda()
        ...
        for y in range(output.shape[1]):
            target = torch.empty_like(label).fill_(y)
...
def fisher_new(data_loaders, model, criterion, args):
    ...
    hessian(dataset, model, criterion)
    for i, p in enumerate(model.parameters()):
        mu, var = get_mean_var(p, args, False)
        p.data = mu+(var ** 0.5)*torch.empty_like(p.data).normal_()
    return model
